...
function roundToPowerOfTen(num, pow) {
  let rez1 = num / Math.pow(10, pow);
  let rez2 = Math.pow(10, pow);
  const rez = Math.round(rez1) * rez2;
  return rez;
}
...
function toExponential(number, fractionDigits) {
  const rez = number.toExponential(fractionDigits);
  return rez;
}
...
function toFixed(number, fractionDigits) {
  const rez = number.toFixed(fractionDigits);
  return rez;
}
...
function toPrecision(number, precision) {
  const rez = number.toPrecision(precision);
  return rez;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const rez = parseFloat((x1 + x2 + x3).toFixed(10));
  return rez;
}
...
function getMaxNumber(firstNumber, secondNumber) {
  const rez = Math.max(firstNumber, secondNumber);
  return rez;
}
